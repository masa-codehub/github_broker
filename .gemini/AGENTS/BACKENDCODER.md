...
  - **ブランチ戦略の遵守**: Issueの指示に従い、適切なベースブランチから作業ブランチを作成します。特に、直前の作業から派生したリファクタリングやドキュメント修正のようなフォローアップタスクでは、`main`ではなく直前の作業ブランチをベースとすることを原則とします。これにより、関連する変更をフィーチャーブランチに集約します。
...
  - **依存関係追加時の型チェック**: 新しい外部ライブラリを依存関係に追加する際は、`mypy`の型チェックでエラーが発生する可能性を考慮します。必要に応じて、`pyproject.toml`の`[tool.mypy]`セクションに`ignore_missing_imports = true`などの設定を追加し、コミット時のCIエラーを未然に防いでください。
...
- **【最優先】ブランチの同期:** いかなる分析よりも先に、まず `git pull --rebase origin <base_branch_name>` を実行し、作業ブランチをベースブランチの最新状態に更新します。
...
- **広範なコンテキストの調査:** 依存関係、ビルドプロセス、CI/CDに関わる変更の場合、`pyproject.toml`、`.github/workflows/*.yml`、関連ドキュメント（例: `README.md`）を調査し、プロジェクトの既存のセットアップとの整合性を確認する。
- **クリーンアーキテクチャ上の位置特定:**
...
- **TDDサイクルの計画 (Red-Green-Refactor):**
    - **Red:** 最初に書くべき「失敗するテスト」を具体的に定義します。それは何を検証するテストか？どのような名前にすべきか？バグ修正やバリデーションのタスクでは、まず最初に問題を再現する最小の失敗テストを作成することから始める。
...
- **TDDサイクルの実行:**
    ...
    - **Refactor（仮説検証）:**
        ...
        2. 直ちに **`pre-commit run --all-files`** を実行し、リント、テストを含む全ての品質チェックがパスすることを確認することで、仮説を検証します。
...
- **タスク完了とPR作成:** Issueの完了条件をすべて満たしたと判断した場合、**最終確認として `pre-commit run --all-files` を実行**し、すべてのチェックがパスすることを確認した後、`git status`から`create_pull_request`までの一連のコマンドを厳密に実行し、プルリクエストを作成して活動報告をIssueに投稿します。
...
- **原則の参照:** ツール実行のエラーに直面した場合、まず共通の行動規範（`_GEMINI.md`）の`セクション7`に定義された「エラー発生時の行動規範参照プロセス」に従い、技術的なデバッグの前に、関連するルールがないか確認すること。
...
- **仮説の転換:** 「三振」した場合、それまでの仮説を破棄し、全く異なる角度から問題の原因を探る。検証すべき仮説の階層は以下の通り。
    ...
    3.  **実行コンテキストの問題:** 「テストの実行順序、環境変数、外部サービスの状態、あるいはビルド設定やCI/CDの規約など、コードの外側に問題があるのではないか？」
