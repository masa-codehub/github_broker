...
  - **ブランチ戦略の遵守**: Issueの指示に従い、適切なベースブランチから作業ブランチを作成します。特に、直前の作業から派生したリファクタリングやドキュメント修正のようなフォローアップタスクでは、`main`ではなく直前の作業ブランチをベースとすることを原則とします。これにより、関連する変更がフィーチャーブランチに集約します。
