...
全エージェントは、John Boydの提唱するOODAループを思考と行動の基本サイクルとして厳格に遵守し、冗長な思考や行動のループを回避しなければなりません。

**思考のループは、エージェントが陥ってはならない最も重大なアンチパターンです。**

1.  **Observe (観察・分析):**
    - **思考の開始:** `Observe:` という接頭辞で思考を開始し、現在のフェーズを明示します。
    - **インプットの収集:**
        - **最優先インプット:** 前の `Act` フェーズの実行結果（成功、失敗、エラーメッセージ、出力内容）を最優先の分析対象とします。**前回の行動結果を無視してはなりません。**
        - **基本インプット:** `.gemini/GEMINI.md` と `~/.gemini/GEMINI.md` の行動規範を常に参照します。
        - **タスクインプット:** 担当Issue、関連ドキュメント、コード、過去の議論など、内外の客観的な事実を収集し、現状と「あるべき姿」とのギャップを正確に把握します。

2.  **Orient (方針決定・計画):**
    - **思考の開始:** `Orient:` という接頭辞で思考を開始します。
    - **原因分析と計画立案:**
        - `Observe` フェーズで得られた情報、特に前回の `Act` の結果を基に、問題の根本原因を分析します。
        - ギャップを解消するための具体的な行動計画を、複数の代替案を含めて検討します。タスクをより小さなステップに分解し、リスクを評価します。
        - **同じ計画の回避:** 前回の `Act` が失敗した場合、**同じ計画やアプローチを再提案してはいけません。** 必ず異なる、より改善された計画を立ててください。

3.  **Decide (意思決定):**
    - **思考の開始:** `Decide:` という接頭辞で思考を開始します。
    - **アクションの選択:** 計画した複数のステップや代替案の中から、次に実行すべき最も効果的で、かつ成功確率が高いと判断されるアクションを一つに絞り込みます。

4.  **Act (実行):**
    - **思考の開始:** `Act:` という接頭辞で思考を開始します。
    - **ツールの実行:** 決定したアクションを、利用可能なツールを駆使して実行します。
    - **結果の記録:** 実行後は、その結果（成功、失敗、エラーメッセージ、出力内容）を次のOODAループの `Observe` フェーズに繋げるため、注意深く記録・記憶します。

**思考ループの検知と脱出:**
- 同じ `Act` が複数回連続で失敗した場合、または同じような `Observe` -> `Orient` の思考が続いていると自己認識した場合は、現在のOODAループが非生産的であると判断します。
- その場合、直ちにループを中断し、`Orient` フェーズに戻って全く異なる視点から計画を立て直すか、あるいは `Decide` フェーズでユーザーに助けを求める（質問する）というアクションを選択してください。
