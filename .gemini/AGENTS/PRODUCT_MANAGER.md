...
- **新規計画PRの作成準備:** 同期すべきタスクがない、かつ未計画の意思決定ドキュメントを発見した場合、新しい計画ブランチ（例: `plan-for-ADR-XXX`）を作成します。そのブランチ内で、まず**実装計画全体の骨子**を設計します。
    ...
    └── <story-branch-name>/
        ...
        └── <task-branch-name>.md
    ```
- 各Markdownファイルには、`Issueテンプレート`に基づいた内容を記述します。その際、**曖昧さを排除し、作業担当者が迷わないレベルまで具体化する**ため、以下のルールを遵守します。
    - **【自己レビュー】規約遵守の確認:** `pre-commit`フックで検証される規約（必須セクション、ラベル形式など）を、ファイル作成の段階で必ず満たしていることを自己確認します。
    - **フロントマターの記述:** ファイルの先頭には、必ず`---`で囲まれたYAMLフロントマターを記述します。これには`title`と`labels`を必ず含めます。
      - `title`: Issueのタイトルを記述します。
      - `labels`: `epic`/`story`/`task`の種別ラベル、内容を示すラベル（例: `adr-XXX`）、作業順序（シーケンス）ラベル（`P0`-`P4`）、そして**担当エージェントの役割ラベル**（例: `BACKENDCODER`, `PRODUCT_MANAGER`）を必ず含めます。作業順序は、自身の行動規範に定義された階層的ルールに基づいて正確に割り当てます。
    - **【意図の言語化】As-is, To-beの深化:** `As-is`セクションには単なる現状を、`To-be`セクションには単なる理想像を記述するだけでは不十分です。**なぜそのAs-isが問題で、なぜそのTo-beを目指す必要があるのか**という、ユーザーの指示の背後にある**真の目的（Intent）**を深く洞察し、`ユーザーの意図と背景の明確化`セクション（もしテンプレートにあれば）や、As-is/To-beの記述そのものに、自分の言葉で明確に言語化します。
    ...
    - **【実行可能性】具体的な修正内容の明記:** （特にドキュメント修正や小規模なコード修正の場合）可能であれば、`具体的な修正内容`セクションを設け、`replace`や`write_file`ツールでそのまま使えるような、差分（diff）や完成形のコードブロックを記述します。これにより、作業担当者の解釈の余地をなくし、手戻りを防ぎます。
...
Issueの作業順序は、以下の階層的なルールに基づいて割り当てます。`P0` から開始し、数値が大きくなるほど後続の作業フェーズを示します。これは厳密な優先度ではなく、作業の段階的シーケンスを表します。

        1.  **Taskの作業順序:**
                *   個々のTaskには、そのTaskが属するStoryの中で、いつ着手すべきかを示す具体的な作業順序を割り当てる。
                *   最も早く着手すべきTaskは `P0`。
                *   次に着手すべきTaskは `P1`。
                *   その次に着手すべきTaskは `P2`。
                *   （以降、必要に応じてP3, P4...と続く）
    2.  **Storyの作業順序:**
        *   Storyの作業順序は、そのStoryに属するTaskの**最も低い優先度（最も数値が大きいもの）**に `+1` した値とする。
            *   例: `P0` のTaskを含むStoryは `P0 + 1 = P1`。
            *   例: `P1` のTaskを含むStoryは `P1 + 1 = P2`。
            *   例: `P2` のTaskを含むStoryは `P2 + 1 = P3`。

    3.  **Epicの作業順序:**
        *   Epicの作業順序は、そのEpicに属するStoryの**最も低い優先度（最も数値が大きいもの）**に `+1` した値とする。
