...
- 各Markdownファイルには、`Issueテンプレート`に基づいた内容を記述します。その際、**曖昧さを排除するため、以下のルールを遵守します。**
    - **フロントマターの記述:** ファイルの先頭には、必ず`---`で囲まれたYAMLフロントマターを記述します。これには`title`と`labels`を必ず含めます。
      ...
      - `labels`: `epic`/`story`/`task`の種別ラベル、内容を示すラベル（例: `adr-XXX`）、そして**優先度ラベル（`P0`-`P4`）**を必ず含めます。優先度は、自身の行動規範に定義された階層的ルールに基づいて正確に割り当てます。
...
- **優先度の割り当て:** Issueの優先度は、以下の階層的なルールに基づいて割り当てます。優先度は `P0` が最も高く、数値が大きくなるほど優先度が低くなります。
    ...
    2.  **Storyの優先度:**
        *   Storyの優先度は、そのStoryに属するTaskの**最も高い優先度（最も数値が小さいもの）**に `+1` した値とする。
            *   例: `P0` のTaskを含むStoryは `P0 + 1 = P1`。
            *   例: `P1` のTaskを含むStoryは `P1 + 1 = P2`。
            *   例: `P2` のTaskを含むStoryは `P2 + 1 = P3`。

    3.  **Epicの優先度:**
        *   Epicの優先度は、そのEpicに属するStoryの**最も高い優先度（最も数値が小さいもの）**に `+1` した値とする。
