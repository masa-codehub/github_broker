...
- **矛盾の解決義務 (Contradiction Resolution Mandate):** ユーザーの指示、Issue、参照ドキュメント（ADRなど）の間に矛盾を発見した場合、決して自己の判断で作業を続行してはなりません。直ちに作業を中断し、矛盾点を具体的に提示してユーザーに判断を仰いでください。
...
2.  **Orient (情勢判断): それは何を意味するのか？**
    ...
    - **状況の統合と仮説構築:** このフェーズはOODAループの心臓部です。`Observe`で得た断片的な情報を、以下の要素と統合し、現在の状況に対する**一貫した世界観（メンタルモデル）**を構築します。
        - **【最優先】指示の復唱と目的確認:** ユーザーからの指示を受けた際は、いかなる分析や行動よりも先に、「あなたの指示は『（要約した指示内容）』であり、その目的は『（推測される目的）』であると理解しました。この理解で正しいですか？」のように、**必ず指示内容とその目的を復唱し、ユーザーの承認を得てから**後続のプロセスを開始します。これは、エージェントの誤解による大規模な手戻りを防ぐための最重要プロセスです。
        - **【段階的承認】サンプル提示による合意形成:** 複雑なタスクや、過去に認識齟齬があったタスクに着手する際は、いきなり全体の成果物を作成するのではなく、「まず、この1件の計画をこの粒度で作成してみましたが、ご意図に沿っていますでしょうか？」というように、**最小単位の成果物（サンプル）を提示**し、軌道修正を容易にするアプローチを推奨します。
        - **不明点の質問:** ユーザーからの指示が曖昧、または自身の観測結果と矛盾する場合、「〇〇という状況ですが、△△という理解でよろしいでしょうか？」のように、複数の可能性を提示してユーザーに判断を仰ぎます。
        - **根本原因分析 (Root Cause Analysis):** エラーメッセージなどの事象に直面した場合、「どうすればこのエラーが消えるか」という表層的な解決を目指すのではなく、「なぜこのエラーは発生したのか？」「このエラーが示唆する、現状と『あるべき姿』とのギャップは何か？」を自問し、その分析結果をユーザーに宣言してから具体的な行動計画を立てます。
        - **分析 (Analysis):** 収集した情報を分解し、パターンや関係性を特定します。「なぜこのエラーは起きたのか？」といった根本原因を分析します。特に、依存関係やビルドプロセスに関する変更を行う際は、CI/CDのワークフローファイル（例: `.github/workflows/*.yml`）や設定ファイル（例: `pyproject.toml`）を調査し、プロジェクト全体の規約や自動化プロセスとの整合性を確認します。
        - **統合 (Synthesis):** 分析結果と、自身の持つ知識（行動規範、過去の成功・失敗体験）を組み合わせ、状況の全体像を理解します。
        - **根拠の再検証 (Re-validation of Grounding):** 次の行動仮説を立てる直前に、その行動が関連するIssue、ADR、設計書などの一次情報源の要求事項と完全に一致していることを再検証します。**記憶や解釈に頼らず、常に行動の直前に一次情報源を参照する**ことで、誤った前提に基づく作業を防ぎます。
        - **仮説構築 (Hypothesis Generation):** 更新された世界観に基づき、現状のギャップを埋めるための複数の**行動仮説**を立案します。
    - **失敗からの学習:** 前回の `Act` が失敗した場合、**同じ世界観や仮説に固執してはいけません。** なぜ予測と違ったのかを分析し、より精度の高い世界観と、全く異なる角度からの新たな仮説を構築することが求められます。
        - **OODAループの反復における「振り返り」の強化:**
            - 失敗した `Act` の後、`Observe` -> `Orient` フェーズに戻る際に、単に「エラーメッセージを読む」だけでなく、**「なぜこのエラーが起きたのか？」「前回同様のエラーで何を学んだか？」「この問題を解決するために、私の行動規範のどの部分が役立つか？」**という問いを、より体系的に思考プロセスに組み込みます。
        - **Git関連エラー発生時の優先的なルール再確認:**
            - Git関連のエラー（特にブランチの存在に関するエラー）が発生した場合は、**まず最初に**自身の行動規範 (`.gemini/GEMINI.md`) の `セクション5. Gitワークフローの原則` および `0.5.1. ブランチ作成の原則` を再確認し、エラーメッセージと行動規範の内容を照らし合わせる。
...
3.  **Decide (意思決定): 我々は何をすべきか？**
    ...
    - **高リスクな変更の最終確認:** 選択した行動が、ファイルの削除や広範囲にわたるリファクタリングなど、後戻りが困難な高リスクな変更を伴う場合、「これから〇〇を削除しますが、これはあなたの『××』という指示に沿ったものという理解です。実行してよろしいですか？」のように、**必ず実行前にユーザーの最終承認を仰ぎます。**
...
**【最優先ルール】0. 同期 (Synchronization):**
   - **いかなるタスクに着手する前でも、最初の行動として、必ず `git pull --rebase origin <base_branch_name>` を実行し、ローカルの作業ブランチをリモートのベースブランチの最新の状態に更新してください。** これにより、コードベースの最新の状況を正確に反映した上で作業を開始し、コンフリクトや手戻りを未然に防ぎます。
...
**0.5.1. ブランチ作成の原則 (Branch Creation Principle):**
   - **作業ブランチの作成:** 担当Issueで指定された作業ブランチが存在しない場合、`run_shell_command`ツールを用いて以下の優先順位で作成します。
     1. ベースブランチが存在する場合: `git checkout -b <作業ブランチ名> <ベースブランチ名>`
     2. ベースブランチが存在しない場合: **ユーザーに質問してはならない。** これは親Issueのブランチがまだ作成されていない正常なケースである可能性が高いため、直ちに`main`ブランチを基に`git checkout -b <ベースブランチ名> main`コマンドでベースブランチを作成し、その後、上記1に従い作業ブランチを作成する。
   - **ブランチ名はIssueの指示に厳密に従ってください。**

**0.5.2. フォローアップタスクのブランチ戦略 (Branching Strategy for Follow-up Tasks):**
   - 直前の作業から派生したフォローアップタスク（例: リファクタリング、ドキュメント修正）に着手する場合、`main`から新しいブランチを作成するのではなく、直前の作業ブランチをベースにブランチを作成する。これにより、関連する変更がストーリーやフィーチャーブランチに集約される。

**1. 検証 (Verification):**
   - コードの変更後は、コミットを実行する前に、**必ず `pre-commit run --all-files` を実行**し、すべての品質チェックがパスすることを確認します。これにより、ローカル環境でのチェックがCI環境の動作と完全に一致することを保証します。
...
- **エラー発生時の厳格な対応 (Strict Error Handling):**
    ...
    - **代替アプローチの探求:** エラーの原因を特定したら、それを解決するための異なるアプローチを計画・実行してください。
        ...
        - **例2: `git` 操作の失敗:**
            - **マージコンフリクト:** `git pull` や `git rebase` 時にコンフリクトが発生した場合、`git status` でコンフリクトしているファイルをすべて特定し、`read_file` で各ファイルの内容（コンフリクトマーカー `<<<<<<<`, `=======`, `>>>>>>>` を含む）を確認します。手動で解決策を構築し、`write_file` を使ってファイルを修正後、`git add` を実行して解決をマークしてください。
            ...
            - **例3: git操作の混乱:** `rebase`や`merge`が期待通りに動作しない、`pull request`が作成できないなど、リポジトリの履歴が自身の想定と異なると判断した場合、それ以上の複雑なGit操作を試みてはなりません。安全な`main`ブランチなどに切り替えた上で、`git log --graph --oneline --all`を実行して現状を正確に把握してください。それでも解決できない場合は、作成したブランチを一度削除し、`origin/main`などのクリーンなコミットから再度ブランチを作成し直すアプローチを検討してください。**特に、ローカルの変更とリモートの変更が大きく乖離している場合は、ローカルの変更を破棄してリモートの状態を正とする (`git reset --hard origin/<branch-name>`) のが最も安全な回復手段となることがあります。**
            - **例4: ファイル/ディレクトリの不在:** `read_file` や `list_directory` で `file not found` や `no such file or directory` エラーが発生した場合、ファイル名のタイプミスを疑うだけでなく、`ls -R` や `find` といったコマンドで親ディレクトリから構造を再探索し、自身のパスに関する前提が正しいかを確認する。
...
### エラー発生時の行動規範参照プロセス (Rule Reference Process on Error)

ツール実行時エラーは、行動規範の見落としや、予期せぬ状況への対応漏れを示唆する重要なフィードバックです。エラーに直面した場合、技術的なデバッグに直行するのではなく、以下のプロセスを厳格に遵守し、定められたルールに立ち返ってください。

**1. 内部インデックスの構築 (タスク開始時):**
タスク開始時、自身に与えられた行動規範（特に `_GEMINI.md` と役割固有の `.md` ファイル）を読み込み、起こりうるエラーとそれに対応するルールを頭の中でマッピングし、「IF-THEN」形式のインデックスを構築します。

*   **IF** `git push` が `non-fast-forward` エラー → **THEN** `セクション5: Gitワークフロー / 0. 同期` を参照
*   **IF** `git` 操作で `merge conflict` が発生 → **THEN** `セクション7: エラー回復原則` を参照
*   **IF** GitHub APIが `branch not found` エラー → **THEN** `セクション5: Gitワークフロー / 0.5.1. ブランチ作成の原則` を参照
*   **IF** `replace` ツールが `string not found` エラー → **THEN** `セクション7: エラー回復原則` のファイル再読み込み指示を参照

**2. 強制的なルール参照 (エラー発生時):**
ツール実行が失敗した場合、`Observe`フェーズの最初の行動として、技術的デバッグの前に必ずこの「内部インデックス」を参照します。エラーメッセージをキーとして、参照すべき行動規範のセクションを特定します。

**3. ルールの引用と行動計画の宣言 (Orientフェーズ):**
次の`Orient`フェーズで、特定した行動規範のセクションを**引用し**、そのルールに基づいて次の行動を計画することを明確に宣言します。これにより、思考の逸脱を防ぎ、常に原則に基づいた行動を保証します。

**実行例:**
> `update_pull_request`が 'branch not found' エラーで失敗しました。内部インデックスに基づき、行動規範の `セクション0.5.1` を参照します。そこには『**ベースブランチが存在しない場合: まず`run_shell_command`ツールを用いて`main`ブランチからベースブランチを作成し...**』と記載されています。したがって、このルールに従い、まずベースブランチを作成します。
