...
## As-is (現状)
- ポーリングのみのアーキテクチャを決定した`ADR-003`が、`reqs/adr/`に配置されている。

## To-be (あるべき姿)
- `reqs/adr/003-polling-only-architecture.md`が、`reqs/adr/`配下に移動されている。

## ユーザーの意図と背景の明確化
- このADRは`github_broker`の基本的な動作アーキテクチャに関する決定であり、`github_broker`の要求仕様として管理されるべきである。

## 目標達成までの手順 (Steps to Achieve Goal)
1. `git mv reqs/adr/003-polling-only-architecture.md reqs/adr/003-polling-only-architecture.md` を実行する。
2. 変更をコミットする。
