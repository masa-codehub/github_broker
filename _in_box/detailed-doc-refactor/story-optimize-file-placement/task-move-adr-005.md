...
## As-is (現状)
- 複数リポジトリのサポートを決定した`ADR-005`が、`reqs/adr/`に配置されている。

## To-be (あるべき姿)
- `reqs/adr/005-multi-repository-support.md`が、`reqs/adr/`配下に移動されている。

## ユーザーの意図と背景の明確化
- このADRは`github_broker`の主要機能に関する決定であり、`github_broker`の要求仕様として管理されるべきである。

## 目標達成までの手順 (Steps to Achieve Goal)
1. `git mv reqs/adr/005-multi-repository-support.md reqs/adr/005-multi-repository-support.md` を実行する。
2. 変更をコミットする。
