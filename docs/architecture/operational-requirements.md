...
-   **ヘルスチェックエンドポイント:**
    `/health`エンドポイントが提供されており、アプリケーションが正常に動作しているかを確認できます。このエンドポイントは、アプリケーションプロセスが起動している場合に `200 OK` を返します。
    -   **エンドポイント:** `/health`
    -   **メソッド:** `GET`
    -   **レスポンス:** `{"status": "ok"}`
-   **メトリクス:**
...
-   **今後の改善予定:**
    -   ヘルスチェックにおいて、主要な依存サービス（GitHub, Redis）への接続確認を行う機能を追加する。
    -   必要に応じて、より詳細なヘルス情報（例: 外部サービスの応答時間、キューの滞留状況など）を返却する機能を検討する。
