...
#### 4.1. 責務の明確化 (クライアントサイドプロンプト生成)

本アーキテクチャ変更により、プロンプト生成の責務は**クライアント側（ワーカーエージェント）**に移管されます。これにより、サーバーはタスクの割り当てと状態管理に専念し、プロンプト生成の柔軟性と環境依存の問題をクライアント側で吸収します。

*   **サーバーの責務:**
    *   Issueの内容、エージェントの役割、過去の対話履歴などに基づき、**プロンプト生成に必要なコンテキスト情報**を `/request-task` APIのレスポンスとしてクライアントに提供する。
    *   プロンプト文字列そのものは生成しない。

*   **クライアントの責務:**
    *   サーバーから受け取ったコンテキスト情報（Issueの内容、ブランチ名など）と、自身の持つプロンプトテンプレートを組み合わせて、**LLMに渡す最終的なプロンプト文字列を生成する。**
    *   生成したプロンプトをLLMに渡し、タスクを実行する。
    *   プロンプト生成ロジックを持つことで、エージェントはより柔軟なタスク実行が可能となる。
