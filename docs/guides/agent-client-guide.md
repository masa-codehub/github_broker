...
`AgentClient`は、タスク割り当てサーバーに新しいタスクを要求するためのHTTPクライアントであり、サーバーから受け取ったプロンプトを直接実行する**シンクライアント**としての責務も担います。
...
1.  **タスクが正常に割り当てられた場合**: タスク情報を含む`dict`（辞書）を返します。この際、`AgentClient`は受け取った`prompt`フィールドのコマンドを内部で自動的に実行します。
