...
2.  **ブローカーによる選択 (厳格な優先度バケット方式):**
    ...
    -   ブローカーは、現在オープンなIssueの中で**最も高い優先度レベル**を持つIssueのみをタスク候補とします。最高優先度レベルのIssueが一つでもオープンである限り、それより低い優先度レベルのIssueは、エージェントに割り当てられることはありません（[ADR-015](./adr/015-strict-priority-bucket-assignment.md)参照）。
