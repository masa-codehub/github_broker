...
この設計により、プロンプトの生成ロジックはサーバーサイドにカプセル化され、クライアントは受け取ったプロンプトを実行するだけのシンプルな責務に集中できます。
>>>>>>> afbc997 (docs: Document code correction prompt specification in gemini-executor-guide.md)
>>>>>>> e4741da (docs: Document code correction prompt specification in gemini-executor-guide.md)
