...
### 5.3. エージェント設定ファイルのパス (`AGENT_CONFIG_PATH`) の設定 (オプション)

エージェントの役割定義を外部ファイルで管理する場合に、そのYAMLファイルのパスを指定します。

1.  **`agents.yml` ファイルの作成**:
    プロジェクトのルートディレクトリに `agents.yml` ファイルを作成し、以下のフォーマットでエージェントの役割を定義します。

    ```yaml
    # agents.yml (例)
    agents:
      - role: BACKENDCODER
        description: "バックエンド開発タスクを担当するエージェント"
      - role: CONTENTS_WRITER
        description: "ドキュメントやコンテンツの作成を担当するエージェント"
    ```

2.  **`.env` ファイルへの追加 (任意)**:
    デフォルトでは `/app/agents.yml` が使用されます。開発環境ではプロジェクトルートがコンテナ内の `/app` にマウントされるため、プロジェクトルートに配置した `agents.yml` が自動的に読み込まれます。別のパスを使用したい場合は、`.env` ファイルに以下の行を追加します。

    ```ini
    # .env
    AGENT_CONFIG_PATH=/path/to/your/agents.yml
    ```
    
    **注意**: この設定は、アプリケーションがタスクをどのエージェントに割り当てるかを決定するために使用されます。新しいエージェントを追加したり、既存のエージェントの役割を変更したりする場合は、このYAMLファイルを更新してください。
