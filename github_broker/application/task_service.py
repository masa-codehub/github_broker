...
BRANCH_PATTERN = re.compile(
    r'## ブランチ名\s*(.+?)\s*(?:##|$)', re.IGNORECASE | re.DOTALL)
DELIVERABLES_PATTERN = re.compile(
    r'## 成果物\s*(.+?)\s*(?:##|$)', re.IGNORECASE | re.DOTALL)
...
class TaskService:
    ...
    def _parse_issue_body(self, body: str | None) -> tuple[str | None, str | None]:
        ...
        deliverables = deliverables_match.group(
            1).strip() if deliverables_match else None
    ...
    def request_task(self, agent_id: str, capabilities: list[str]) -> TaskResponse | None:
        if not self._redis_client.acquire_lock():
            raise LockAcquisitionError("Failed to acquire lock. Server is busy.")
        ...
        try:
            ...
            if previous_task_processed:
                logging.info(
                    f"前回のタスクを処理しました。GitHub検索インデックスの更新を {self.GITHUB_INDEX_WAIT_SECONDS} 秒待ちます...")
            ...
            try:
                ...
                logging.warning(
                    "An error occurred while trying to select a new issue. It might have been deleted or there was a client error.")
                return None
            ...
            try:
                ...
                self._github_client.add_label(
                    repo_name=self.repo_name, issue_id=issue_number, label="in-progress")
                self._github_client.add_label(
                    repo_name=self.repo_name, issue_id=issue_number, label=agent_id)
                self._github_client.create_branch(
                    repo_name=self.repo_name, branch_name=branch_name)
                ...
                logging.warning(
                    f"Issue #{new_issue.number} not found on GitHub during assignment. It might have been deleted. Skipping.")
                return None
    ...
    def _process_previous_task(self, agent_id: str) -> bool:
        ...
        if previous_issue:
            ...
            logging.info(
                f"Agent {agent_id} completed issue #{previous_issue_number}. Updating labels.")
            try:
                self._github_client.remove_label(
                    repo_name=self.repo_name, issue_id=previous_issue_number, label="in-progress")
                self._github_client.remove_label(
                    repo_name=self.repo_name, issue_id=previous_issue_number, label=agent_id)
                self._github_client.add_label(
                    repo_name=self.repo_name, issue_id=previous_issue_number, label="needs-review")
                return True  # ラベル更新に成功
            except UnknownObjectException:
                logging.warning(
                    f"Error: Previous issue #{previous_issue_number} not found on GitHub during label update.")
                return False  # 更新中にエラー
        else:
            logging.info(
                f"No previous in-progress task found for agent {agent_id}.")
            return False  # 対象のタスクなし

    def _select_best_issue(self, capabilities: list[str], agent_id: str):
        open_issues = self._github_client.get_open_issues(
            repo_name=self.repo_name)
        if not open_issues:
        ...
        logging.info(
            f"Found {len(open_issues)} open issues. Checking for ready tasks...")
        ready_issues = [
            issue for issue in open_issues if self._is_task_ready(issue)]

        if not ready_issues:
            logging.warning(
                f"No ready issues found. All {len(open_issues)} open issues are missing the '## 成果物' section or it is empty.")
            for issue in open_issues:
                _, deliverables = self._parse_issue_body(issue.body)
                branch, _ = self._parse_issue_body(issue.body)
                logging.info(
                    f"  - Issue #{issue.number} ('{issue.title}') is not ready. Branch: {branch}, Deliverables: {deliverables}")
            return None

        # --- 案Aの実装 ---
        logging.info(
            f"Found {len(ready_issues)} ready issues. Selecting the first one.")
        selected_issue = ready_issues[0]
        logging.info(
            f"Selected issue #{selected_issue.number} ('{selected_issue.title}').")
