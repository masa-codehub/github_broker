...
class TaskService:
    ...
    def request_task(self) -> str:
        ...
        if not self.redis_client.acquire_lock(self.lock_key, str(uuid4())):
            ...
            raise LockAcquisitionError("Failed to acquire lock. Another task is likely running.")
    ...
    def get_running_task(self) -> Optional[str]:
        """
    ...
    def release_task(self):
        ...
        if not self.redis_client.release_lock(self.lock_key):
            logger.error("Failed to release lock. It may have expired or been released by another process.")
