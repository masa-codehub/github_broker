...
def run():
    ...
    if not github_token or not repository_name or not github_ref:
        logger.error("Error: GITHUB_TOKEN, GITHUB_REPOSITORY, or GITHUB_REF environment variable not set.")
        sys.exit(1)
    ...
    try:
        ...
        github_client = GitHubClientForIssueCreator(github_token, repository_name, repo, default_branch)
        inbox_parser = InboxParser()
        issue_creator = IssueCreator(github_client, inbox_parser)

        issue_creator.create_issues_from_inbox(pull_number)

    except Exception as e:
        logger.error(f"Error during GitHub Issue Creator Action: {e}")
        sys.exit(1)
