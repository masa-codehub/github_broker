...
prompt_template: >
  ...
  **あなたのタスク:**
  ...
  5. `run_shell_command`ツールを使い、`git pull --rebase origin {base_branch_name}` を実行してベースブランチの最新の変更を取り込む。
  6. 策定した作業計画とあなたの行動規範に従って、作業を完了させる。
  7. もし作業の途中でテスト失敗や予期せぬエラーが発生した場合は、作業計画を見直し、別の解決策を試みること。
  8. `pre-commit run --all-files` を実行し、すべてのチェックがパスすることを確認する。
  9. `run_shell_command`ツールを使い、すべての作業が完了したら変更をコミットする。
  10. プルリクエストを作成する。
...
review_fix_prompt_template: >
  ...
  **あなたのタスク:**
  ...
  4. レビューコメントを完全に理解し、修正作業を計画し、ユーザーに宣言する。
  5. `run_shell_command`ツールを使い、プルリクエストで指定されている作業ブランチに切り替える。
  6. `run_shell_command`ツールを使い、まず、`git pull --rebase origin {base_branch_name}` を実行してベースブランチの最新の変更を取り込み、コンフリクトがあれば解消する。
  7. 策定した作業計画とあなたの行動規範に従って、レビューコメントの指摘事項を修正する。
  8. `pre-commit run --all-files` を実行し、すべてのチェックがパスすることを確認する。
  9. `run_shell_command`ツールを使い、修正が完了したら変更をコミットする。
  10. プルリクエストを更新する。
