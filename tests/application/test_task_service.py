...
class TestTaskService(unittest.TestCase):
    def setUp(self):
        ...
        self.task_service = TaskService(
            redis_client=self.mock_redis_client, github_client=self.mock_github_client
        )
    ...
    def test_request_task_lock_acquisition_fails(self):
        ...
        with pytest.raises(
            ...
            match="Failed to acquire lock. Another task is likely running.",
        ):
    ...
    def test_release_task_fails(self):
        ...
        self.mock_redis_client.release_lock.assert_called_once()
        self.mock_redis_client.delete_key.assert_called_once_with(
            "task_id"
        )  # Still called
