...
async def test_request_task_sets_task_type_to_review_for_needs_review_issue(
    ...
    mock_github_client.get_pull_request_review_comments.return_value = mock_review_comments
...
async def test_find_first_assignable_task_uses_build_code_review_prompt_for_review_task(
    ...
    with caplog.at_level(logging.INFO):
        ...
        # 1. build_code_review_promptが呼び出されたことを検証
        task_service.gemini_executor.build_code_review_prompt.assert_called_once_with(
            pr_url=issue["html_url"], review_comments=mock_review_comments
        )
        
        # 2. build_promptが呼び出されていないことを検証
        task_service.gemini_executor.build_prompt.assert_not_called()
        
        # 3. ログの検証
        log_messages = [record.message for record in caplog.records]
        assert any(
            f"[issue_id={issue_id}] Used gemini_executor.build_code_review_prompt." in m
            for m in log_messages
        )
        assert any(
            f"[issue_id={issue_id}] Task is a review task. Retrieving review comments." in m
            for m in log_messages
        )
        
        # 4. GitHubクライアントの呼び出し検証
        mock_github_client.get_pull_request_review_comments.assert_called_once_with(issue_id)
